...
/// This function:
/// 1. Validates that all IDs exist
/// 2. Updates contents.genre_id to point to primary_id
...
async fn update_contents_genre(
    ...
    for &dup_id in duplicate_ids {
        // First, get affected content IDs before updating
        let contents = sqlx::query!("SELECT id FROM contents WHERE genre_id = ?", dup_id)
            .fetch_all(&mut **tx)
            .await?;

        // Get books associated with those contents
        for content_record in &contents {
