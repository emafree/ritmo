...
let type_record = Type { key: "type.novel".to_string(), ... };
let format = Format { key: "format.epub".to_string(), ... };

println!("{}", get_translation(&role));    // "Author" / "Autore"
println!("{}", get_translation(&lang));    // "Original Language" / "Lingua Originale"
println!("{}", get_translation(&type_record));  // "Novel" / "Romanzo"
println!("{}", get_translation(&format));  // "EPUB (ebook)"
```

## LocalizableError Trait

The `LocalizableError` trait provides i18n support for error messages. It extends `RitmoErr` with a `localized_message()` method that returns translated error messages based on the current locale.

### Why a Separate Trait?

The `ritmo_errors` crate is kept independent of i18n dependencies. The `LocalizableError` trait is defined in `ritmo_db` and extends `RitmoErr` with localization capabilities without modifying the original error type.
...
impl LocalizableError for RitmoErr {
    fn localized_message(&self) -> String {
        match self {
            ...
            // ... all 40 RitmoErr variants
        }
    }
}
```

### Using Error Localization
...
// Original Display still works (uses #[error] attribute)
println!("{}", err);
// Output: "Database not found: /path/to/db"
```

### Error Translation Keys

All error messages are organized by category in translation files:

```yaml
# locales/en.yml
error:
  database:
    not_found: "Database not found: %{path}"
    connection_failed: "Database connection failed: %{error}"
    query_failed: "Database query failed: %{error}"

  file:
    not_found: "File not found: %{path}"
    access_error: "File access error: %{error}"

  ml:
    error: "Machine Learning error: %{error}"
    name_parsing: "Name parsing error: %{error}"
```

**Note**: rust-i18n v3 uses `%{variable}` syntax for variable substitution (not `{variable}`).

### Benefits

1. **Separation of concerns**: `ritmo_errors` remains simple
2. **Opt-in localization**: Use `localized_message()` when needed
3. **Backward compatible**: Original `Display` trait unchanged
4. **Comprehensive coverage**: All 40 RitmoErr variants supported
5. **Easy extension**: Add new error types by implementing a new match arm

### Example in Practice

```rust
fn handle_database_operation() -> Result<(), RitmoErr> {
    // ... operation that might fail
    Err(RitmoErr::DatabaseConnectionFailed("timeout".to_string()))
}
...
fn main() {
    ...
    // Run operation
    if let Err(err) = handle_database_operation() {
        // Show localized error to user
        eprintln!("{}", err.localized_message());

        // Log technical details (English)
        log::error!("{}", err);
    }
}
```
...
The CLI uses i18n for all runtime messages (status, success, error messages). Command help text remains in English to follow standard CLI tool conventions (like git, docker).

### Initializing CLI i18n
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialize i18n system with priority:
    // 1. RITMO_LANG env var (temporary override)
    // 2. Saved preference from config
...
```rust
// ritmo_cli/src/commands/init.rs
use rust_i18n::t;

pub async fn cmd_init(path: Option<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", t!("cli.init.initializing", path = library_path.display().to_string()));
...
### Testing CLI in Different Languages
...
### CLI Message Categories

- **cli.common.*** - Messages used across multiple commands (no library configured, use init, etc.)
- **cli.init.*** - Library initialization messages
- **cli.info.*** - Library information messages
- **cli.list_libraries.*** - List libraries messages
- **cli.set_library.*** - Set library messages
- **cli.language.*** - Language preference management messages

See `locales/en.yml` and `locales/it.yml` for complete CLI message catalog.

## Language Preference Management

Ritmo allows you to save your language preference persistently in the configuration file (`~/.config/ritmo/settings.toml`).

### Setting Language Preference

```bash
# Set language preference (saved to config)
ritmo set-language en    # English
ritmo set-language it    # Italian

# The preference is saved and will be used for all future commands
```
...
# Output shows:
# - Saved preference: The language saved in config
# - Active language: The currently active language
# - Available languages: List of supported languages
# - Environment override notice (if RITMO_LANG is set)
```

### Language Priority Order

The application determines the active language using this priority:

1. **RITMO_LANG** environment variable (temporary override)
   ```bash
   RITMO_LANG=it ritmo info  # Use Italian for this command only
   ```

2. **Saved preference** from `~/.config/ritmo/settings.toml`
   ```bash
   ritmo set-language it     # Save preference permanently
   ```

3. **LANG** environment variable (system default)
   ```bash
   export LANG=it_IT.UTF-8
   ritmo info                # Uses Italian from system locale
   ```
...
# Change to Italian
$ ritmo set-language it
✓ Preferenza lingua salvata: it
...
$ RITMO_LANG=en ritmo get-language
Preferenza salvata: it
Lingua attiva: en
  (overridden by RITMO_LANG environment variable)
...
#[test]
fn test_type_format_translation() {
    use ritmo_db::i18n_utils::set_locale;
    use ritmo_db::{Type, Format};
...
**Phase 1 (Complete):** 54 keys - Infrastructure, Role, RunningLanguages
**Phase 2 (Complete):** 10 keys - Type, Format models
**Phase 3 (Complete):** 48 keys - Error messages (all RitmoErr variants)
**Phase 4 (Complete):** 40 keys - CLI runtime messages (init, info, list-libraries, set-library)
**Phase 5 (Complete):** 6 keys - Language preference management (set-language, get-language)

**Total coverage:** 158 keys (database models + error messages + CLI core commands + language management)
**Remaining:** ~310 keys (additional CLI commands, GUI, service messages)
