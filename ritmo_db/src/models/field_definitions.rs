...
impl FieldDefinitionRow {
    ...
    pub async fn save(&self, pool: &sqlx::SqlitePool) -> Result<i64, sqlx::Error> {
        let result = sqlx::query(
            "INSERT INTO field_definitions (entity, field_name, data_kind, sort_order, enum_values)
             VALUES (?, ?, ?, ?, ?)",
        )
        .bind(&self.entity)
    ...
    pub async fn list_for_entity(
        ...
        sqlx::query_as::<_, FieldDefinitionRow>(
            "SELECT id, entity, field_name, data_kind, sort_order, enum_values, created_at
             FROM field_definitions WHERE entity = ? ORDER BY sort_order",
        )
        .bind(entity)
        .fetch_all(pool)
        .await
