...
pub fn open_book_dialog(
    ...
    let field_names: Vec<slint::SharedString> =
        book_fields.iter().map(|f| f.name.clone()).collect();
    ...
    dialog.set_available_fields(ModelRc::from(Rc::new(
        VecModel::<FieldDefinition>::from(book_fields.clone()),
    )));
    dialog.set_entries(ModelRc::from(Rc::new(
        VecModel::<FieldEntry>::from(vec![first_placeholder(&book_fields)]),
    )));
